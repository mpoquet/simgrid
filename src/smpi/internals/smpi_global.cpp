...
static int smpi_run_entry_point(smpi_entry_point_type entry_point, std::vector<std::string> args, const char * instance_id)
{
...
static void smpi_init_privatization_no_dlopen(std::string executable, const char * instance_id)
{
  ...
  simix_global->default_function = [entry_point, instance_id](std::vector<std::string> args) {
    return std::function<void()>([entry_point, args, instance_id] { smpi_run_entry_point(entry_point, args, instance_id); });
  };
}
